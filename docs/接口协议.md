...
| 客户端 | 套餐 | prompt_word 每日限额 | download 每日限额 | high_value_notes 每日限额 | keyword_expansion 每日限额 |
|--------|------|-----------------|----------------|----------------------|--------------------------|
| browser-extension | basic | 20 | 20 | 30 | 10 |
| browser-extension | premium | 50 | 20 | 100 | 50 |
| browser-extension | vip | 100 | 50 | 200 | 100 |
| pc-client | basic | 30 | 30 | 50 | 20 |
| pc-client | premium | 80 | 50 | 150 | 80 |
| pc-client | vip | 150 | 100 | 300 | 150 |
...
|--------|------|------|------|
| client_type | string | 是 | 客户端类型：`browser-extension` / `pc-client` |
| package_type | string | 是 | 套餐类型：`basic` / `premium` / `vip` |
...
1. 如果 auth_code 未使用过，首次激活时：
   - 如果设备已有其他未过期激活码 → **累计有效期**：原过期时间 + 新激活码天数
   - 如果设备已有过期激活码 → 从**当前激活时间**开始计算
   - 如果设备已有永久激活码 → 直接保持永久
   - 如果设备没有激活记录 → 从当前激活时间开始计算

2. 如果 auth_code 已使用过 → 验证客户端提供的 current_expiry_date 是否与存储一致

3. 更新激活码 duration 后 → 自动根据原始激活时间重新计算过期时间

### 多激活码处理规则
同一个设备允许绑定**多个激活码**，查询时自动按激活时间倒序，返回**第一个未过期**的激活信息。这样用户可以分次购买有效期，自动累计。
