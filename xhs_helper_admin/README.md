# xhs_helper-admin - 小红书助手激活码API服务（腾讯云SCF版）

小红书助手（浏览器插件/PC客户端）激活码验证管理服务，基于腾讯云Serverless云函数Python 3.9实现，使用SQLite文件存储，无需额外数据库服务。

支持浏览器插件和PC客户端区分，三级API Key权限分离，动态权限配额管理。
...
| 变量名 | 说明 | 默认值 | 示例 |
|--------|------|--------|------|
| `ADMIN_API_KEY` | 管理端API密钥（管理接口必须，只有admin可以生成/删除激活码） | - | `your-secure-admin-key` |
| `CLIENT_API_KEYS` | 客户端API密钥（多个用逗号分隔） | - | `client-key-1,client-key-2` |
| `RATE_LIMIT` | 每分钟请求限制 | `60` | `60` |
| `DB_PATH` | SQLite数据库文件路径 | `/tmp/activation.db` | 如果使用CFS，填写CFS挂载路径，比如 `/mnt/cfs/activation.db` |
| `TZ` | 时区设置（推荐设置，让日志时间对得上北京时间） | 可选 | `Asia/Shanghai` |
...
| 接口 | 方法 | 用途 | 需要权限 |
|------|------|------|------------|
| `/auth/verify` | POST | 用户端验证激活码 | client/admin |
| `/device/info` | POST | 查询设备激活状态和权限 | client/admin |
| `/auth/generate` | POST | 管理端生成新激活码 | admin |
| `/auth/list` | POST | 管理端列出所有激活码 | admin |
| `/auth/delete` | POST | 管理端删除激活码 | admin |
| `/auth/update` | POST | 管理端更新激活码信息 | admin |
| `/device/list` | POST | 管理端列出所有设备 | admin |
| `/device/unbind` | POST | 管理端解绑设备 | admin |
| `/device/delete` | POST | 管理端删除设备 | admin |
| `/permissions/list` | POST | 管理端列出所有权限配置 | admin |
| `/permissions/set` | POST | 管理端设置/更新权限配置 | admin |
| `/permissions/delete` | POST | 管理端删除权限配置 | admin |
